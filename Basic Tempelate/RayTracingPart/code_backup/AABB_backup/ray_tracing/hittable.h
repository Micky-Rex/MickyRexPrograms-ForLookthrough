...
namespace BasicRayTracing {
	...
	*/
	...
	public:
		...
		{
			const ray located = transformation.local_ray(r);
	...
	*/
	...
	public:
		...
		sphere(const vec3& _center, double radius, shared_ptr<material> mat) : center(), radius(radius), mat(mat) { transformation = transform(_center); init_bounding_box(); };
	...
	*/
	...
	public:
		...
		triangle(const vec3& p1, const vec3& p2, const vec3& p3, shared_ptr<material> mat) : p1(p1), p2(p2), p3(p3), mat(mat), e1(p2 - p1), e2(p3 - p1) { outward_normal = normalize(cross(e2, e1)); init_bounding_box(); }
