...
namespace BasicRayTracing2D {
	...
	*/
	...
	private:
		...
		vec3 transform_point(const vec3& p) const
		{
			...
			vec3 scaled = p * scale;
			vec3 rotated = vec3(
				dot(rotation_matrix[0], scaled),
				dot(rotation_matrix[1], scaled),
				dot(rotation_matrix[2], scaled)
			);
		...
		vec3 inverse_transform_point(const vec3& p) const
		{
			...
			vec3 unrotated = vec3(
				dot(rotation_matrix[0], untranslated),
				dot(rotation_matrix[1], untranslated),
				dot(rotation_matrix[2], untranslated)
			);
		...
		vec3 inverse_transform_vec(const vec3& v) const
		{
			update_rotation();
			vec3 unrotated = vec3(
				dot(rotation_matrix[0], v),
				dot(rotation_matrix[1], v),
				dot(rotation_matrix[2], v)
			);
			return unrotated / scale;
		}
	};
	...
	*/
	...
	public:
		...
		void set_position(const vec3& pos) { center = pos; }
	...
	*/
	...
	public:
		triangle(const vec3& pos, const vec3& p1, const vec3& p2, const vec3& p3, shared_ptr<material> mat) : transformation(pos), p1(p1), p2(p2), p3(p3), mat(mat), e1(p2 - p1), e2(p3 - p1) {};
