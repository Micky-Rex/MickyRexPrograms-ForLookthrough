...
dvec3 at(const ray r, double t) {
	return dvec3(r.origin + t * r.direction);
}

void located_ray(inout ray r, const dmat4 inverse_matrix) {
...
void set_face_normal(inout hit_record rec, ray r, dvec3 outward_normal) {
...
bool scatter_lambertian(const material mat, const ray r_in, inout hit_record rec, inout vec3 attenuation, inout ray scattered) {
	dvec3 scatter_direction = rec.normal + random_unit_vector();
	...
	attenuation = value(mat.tex, float(rec.u), float(rec.v), vec3(rec.p));
	return true;
}
...
bool _hit_located_ray(const aabb bbox, const ray r, interval ray_t) {
	const float adinv_x = 1.0 / float(r.direction.x);
	const float adinv_y = 1.0 / float(r.direction.y);
	const float adinv_z = 1.0 / float(r.direction.z);

	const float t0x = (bbox.x.min - float(r.origin.x)) * adinv_x;
	const float t1x = (bbox.x.max - float(r.origin.x)) * adinv_x;
	const float t0y = (bbox.y.min - float(r.origin.y)) * adinv_y;
	const float t1y = (bbox.y.max - float(r.origin.y)) * adinv_y;
	const float t0z = (bbox.z.min - float(r.origin.z)) * adinv_z;
	const float t1z = (bbox.z.max - float(r.origin.z)) * adinv_z;
...
void get_uv_sphere(const dvec3 p, inout double u, inout double v) {
	float theta = acos(float(-p.y));
	float phi = atan(float(-p.z), float(p.x)) + pi;
	u = phi / (2 * pi);
	v = theta / pi;
	return;
}
...
bool _hit_located_ray_sphere(const hittable sphere, const ray r, interval ray_t, inout hit_record rec) {
    //return true;
	dvec3 oc = sphere.center - r.origin;
	...
	rec.t = root;
	rec.p = at(r, rec.t);
	dvec3 outward_normal = (rec.p - sphere.center) / sphere.radius;
...
vec3 ray_color(ray r, const hittable_list world, int depth) {
	...
	for (int i = depth; i > 0; i--) {
		if (!hit(world, r, interval(1e-5f, 1e8f), rec)) {
			float a = 0.5f * (normalize(vec3(r.direction)).y + 1.f);
			result_color += current_attenuation * ((1.f - a) * vec3(1.f, 1.f, 1.f) + a * vec3(0.5f, 0.7f, 1.f));//cam.background.xyz;
			break;
		}
		//result_color = dvec3(1,0,0);
		//break;
		vec3 emitted = emit(rec.mat, float(rec.u), float(rec.v), vec3(rec.p));
...
void init()
{
	...
	double focal_length = length(cam.look_from - cam.look_at);
	double theta = radians(cam.vfov);
	double h = tan(theta / 2.f);
	...
	dvec3 viewport_u = viewport_width * cam.u;
	dvec3 viewport_v = viewport_height * cam.v;
	cam.pixel_delta_u = viewport_u / cam.image_width;
	cam.pixel_delta_v = viewport_v / cam.image_height;
	dvec3 viewport_upper_left = cam.look_from - cam.w * focal_length - viewport_u / 2.f - viewport_v / 2.f;
	cam.pixel00_center = viewport_upper_left + cam.pixel_delta_u * 0.5f + cam.pixel_delta_v * 0.5f;
	return;
}
*/
dvec3 sample_square() {
	return dvec3(rand() - 0.5f, rand() - 0.5f, 0.f);
}

ray get_ray(const int i, const int j) {
	dvec3 offset = sample_square();
	dvec3 pixel_sample = cam.pixel00_center + cam.pixel_delta_u * (i + offset.x) + cam.pixel_delta_v * (j + offset.y);
	dvec3 direction = normalize(pixel_sample - cam.look_from);
...
/*
	...
	cam.vup = dvec3(0.f, 1.f, 0.f);
	cam.vfov = 90.f;
	cam.samples_per_pixel = 50;
	cam.aspect_ratio = 16.f/9.f;
	cam.look_from = dvec3(0.0, 0.0, 0.0);
	cam.look_at = dvec3(0.0, 0.0, -1.0);
...
*/
    ...
    ray r = get_ray(x, y);
    //dvec3 col = ray_color(r, world, 50);
